...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .patient-card.relocated{background:#f8fafc;border-style:dashed;}
  .patient-main{flex:1;min-width:0;}
  .patient-details.relocated .patient-name,.patient-details.relocated .patient-meta,.patient-details.relocated .patient-time{opacity:.68;text-decoration:line-through;}
  ...
  .patient-status{display:inline-flex;align-items:center;gap:6px;margin-top:7px;padding:5px 10px;border-radius:999px;background:#dbeafe;color:var(--blue-dark);font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.04em;}
...
function renderPatientList(session) {
  ...
  for (var i = 0; i < appts.length; i++) {
    ...
    var relocated = a.status === 'RELOCATED_SPINOLA';
    html += '<div class="patient-card' + (relocated ? ' relocated' : '') + '">';
    html += '<div class="patient-main">';
    html += '<div class="patient-details' + (relocated ? ' relocated' : '') + '">';
    ...
    html += '</div>';
    html += '</div>';
    if (relocated) html += '<div class="patient-status">Sent to Spinola</div>';
    html += '</div>';
