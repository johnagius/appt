...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .patient-copy-muted{opacity:.68;text-decoration:line-through;}
...
function renderPatientList(session) {
  ...
  for (var i = 0; i < appts.length; i++) {
    ...
    html += '<div class="patient-details' + (relocated ? ' patient-copy-muted' : '') + '">';
    ...
    html += '</div>';
    html += '<div class="patient-time' + (relocated ? ' patient-copy-muted' : '') + '">' + esc(a.startTime || '') + '</div>';
    html += '</div>';
  }
  el.innerHTML = html;
}
