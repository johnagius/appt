...
export async function insertAppointment(db: D1Database, appt: Appointment): Promise<void> {
  await db.prepare(`
    INSERT INTO appointments (id, date_key, start_time, end_time, service_id, service_name,
      ...
      token, calendar_event_id, cancelled_at, cancel_reason, reminder_sent, confirmed, booking_source, hotel)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    ...
    appt.reminder_sent || '', appt.confirmed || '', appt.booking_source || '', appt.hotel || ''
  ).run();
}
