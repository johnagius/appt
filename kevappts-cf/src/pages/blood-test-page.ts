...
export function bloodTestPage(env: Env): string {
...
<title>Blood Tests — ${location}</title>
<link rel="icon" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' rx='20' fill='%23dc2626'/%3E%3Ctext x='50' y='68' font-size='50' font-weight='bold' text-anchor='middle' fill='white' font-family='Arial'%3EBT%3C/text%3E%3C/svg%3E">
<style>
  :root{--accent:#dc2626;--good:#10b981;--bad:#ef4444;--muted:#6b7280;--text:#111827;--line:#e5e7eb;}
  *{box-sizing:border-box}
  ...
  .priceTag{display:inline-block;background:#fee2e2;color:#991b1b;padding:3px 10px;border-radius:999px;font-weight:800;font-size:13px;}
...
<div class="wrap">
  ...
  <div class="card fields">
    ...
    <div><p class="label">Comments (optional)</p><textarea id="fComments" placeholder="Anything the pharmacy staff should know — fasting, medication, prior tests, etc."></textarea></div>
    <button id="submitBtn" class="submit" disabled>Book blood test</button>
...
<script>
(function(){
  var state = { dateOptions: [], selectedDate: '', slots: [], selectedSlot: '', priceCents: 0, types: [] };
  ...
  function renderConfig(c){
    if (c && c.priceCents > 0) {
      ...
      $('priceTbd').style.display = 'block';
    }
    if (c && Array.isArray(c.types) && c.types.length) {
      state.types = c.types;
      var w = $('typeWrap'); w.style.display = 'block';
      var sel = $('fType');
      sel.innerHTML = '';
      var empty = document.createElement('option');
      empty.value = ''; empty.textContent = 'Choose a test (optional)';
  ...
  async function loadInit(){
    try {
      var res = await fetch('/api/blood-test-init');
      var data = await res.json();
      if (data && data.config && data.config.enabled === false) {
        showMsg('Blood test bookings are not currently open. Please check back later.', 'err');
  ...
  async function loadSlots(dateKey){
    renderSlots([], 'Loading…');
    try {
      var res = await fetch('/api/blood-test-availability?date=' + encodeURIComponent(dateKey));
      var data = await res.json();
      if (data.ok) {
  ...
  $('submitBtn').addEventListener('click', async function(){
    ...
    try {
      var body = {
        ...
        testType: ($('fType') && $('fType').value || '').trim(),
      };
      var res = await fetch('/api/blood-test-book', {
        method: 'POST',
      ...
      var data = await res.json();
      if (data.ok) {
        showMsg('Blood test confirmed for ' + data.dateKey + ' at ' + data.startTime + '. Redirecting…', 'ok');
        setTimeout(function(){ window.location.href = data.redirectUrl || '/'; }, 2500);
      } else {
        showMsg(data.reason || 'Booking failed.', 'err');
        $('submitBtn').disabled = false;
        $('submitBtn').textContent = 'Book blood test';
      }
    } catch(e) {
